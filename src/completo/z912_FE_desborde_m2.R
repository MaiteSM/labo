...
agrego_NA <- function(dataset){
	dataset[, ifelse(ccuenta_corriente == 0, mcuenta_corriente_adicional := NA, mcuenta_corriente_adicional := mcuenta_corriente_adicional)]
	dataset[, ifelse(ccuenta_corriente == 0, mcuenta_corriente := NA, mcuenta_corriente := mcuenta_corriente)]
	dataset[, ifelse(ccaja_ahorro == 0, mcaja_ahorro := NA, mcaja_ahorro := mcaja_ahorro)]
	dataset[, ifelse(ccaja_ahorro == 0, mcaja_ahorro_adicional := NA, mcaja_ahorro_adicional := mcaja_ahorro_adicional)]
	dataset[, ifelse(ctarjeta_debito == 0, ctarjeta_debito_trx := NA, ctarjeta_debito_trx := ctarjeta_debito_trx)]
	dataset[, ifelse(ctarjeta_debito == 0, mautoservicio := NA, mautoservicio := mautoservicio)]
	...
	dataset[, ifelse(ctarjeta_master == 0, ctarjeta_master_trx := NA, ctarjeta_master_trx)]
	dataset[, ifelse(ctarjeta_master == 0, mtarjeta_master_consumo := NA, mtarjeta_master_consumo)]
	dataset[, ifelse(cprestamos_personales == 0, mprestamos_personales := NA, mprestamos_personales)]
	dataset[, ifelse(cprestamos_prendarios == 0, mprestamos_prendarios := NA, mprestamos_prendarios)]
	dataset[, ifelse(cprestamos_hipotecarios == 0, mprestamos_hipotecarios := NA, mprestamos_hipotecarios)]
	dataset[, ifelse(cplazo_fijo == 0, mplazo_fijo_dolares := NA, mplazo_fijo_dolares)]
	dataset[, ifelse(cplazo_fijo == 0, mplazo_fijo_pesos := NA, mplazo_fijo_pesos)]
	dataset[, ifelse(cinversion1 == 0, minversion1_pesos := NA, minversion1_pesos)]
	dataset[, ifelse(cinversion1 == 0, minversion1_dolares := NA, minversion1_dolares)]
	dataset[, ifelse(cinversion2 == 0, minversion2 := NA, minversion2)]
	dataset[, ifelse(cpayroll_trx == 0, mpayroll := NA, mpayroll)]
	dataset[, ifelse(cpayroll_trx == 0, mpayroll2 := NA, mpayroll2)]
	dataset[, ifelse(cpayroll_trx == 0, cpayroll2_trx := NA, cpayroll2_trx)]
	dataset[, ifelse(ccuenta_debitos_automaticos == 0, mcuenta_debitos_automaticos := NA, mcuenta_debitos_automaticos)]
	dataset[, ifelse(ctarjeta_visa_debitos_automaticos == 0, mtarjeta_visa_debitos_automaticos := NA, mtarjeta_visa_debitos_automaticos)]
	dataset[, ifelse(ctarjeta_master_debitos_automaticos == 0, mttarjeta_master_debitos_automaticos := NA, mttarjeta_master_debitos_automaticos)]
	dataset[, ifelse(cpagodeservicios == 0, mpagodeservicios := NA, mpagodeservicios)]
	dataset[, ifelse(cpagomiscuentas == 0, mpagomiscuentas := NA, mpagomiscuentas)]
	dataset[, ifelse(ccajeros_propios_descuentos == 0, mcajeros_propios_descuentos := NA, mcajeros_propios_descuentos)]
	dataset[, ifelse(ctarjeta_visa_descuentos == 0, mtarjeta_visa_descuentos := NA, mtarjeta_visa_descuentos)]
	dataset[, ifelse(ctarjeta_master_descuentos == 0, mtarjeta_master_descuentos := NA, mtarjeta_master_descuentos)]
	dataset[, ifelse(ccomisiones_mantenimiento == 0, mcomisiones_mantenimiento := NA, mcomisiones_mantenimiento)]
	dataset[, ifelse(ccomisiones_otras == 0, mcomisiones_otras := NA, mcomisiones_otras)]
	dataset[, ifelse(cforex == 0, cforex_buy := NA, cforex_buy)]
	dataset[, ifelse(cforex == 0, mforex_buy := NA, mforex_buy)]
	dataset[, ifelse(cforex == 0, cforex_sell := NA, cforex_sell)]
	dataset[, ifelse(cforex == 0, mforex_sell := NA, mforex_sell)]
	dataset[, ifelse(cforex_buy == 0, mforex_buy := NA, mforex_buy)]
	dataset[, ifelse(cforex_sell == 0, mforex_sell := NA, mforex_sell)]
	dataset[, ifelse(ctransferencias_recibidas == 0, mtransferencias_recibidas := NA, mtransferencias_recibidas)]
	dataset[, ifelse(ctransferencias_emitidas == 0, mtransferencias_emitidas := NA, mtransferencias_emitidas)]
	dataset[, ifelse(cextraccion_autoservicio == 0, mextraccion_autoservicio := NA, mextraccion_autoservicio)]
	dataset[, ifelse(ccheques_depositados == 0, mcheques_depositados := NA, mcheques_depositados)]
	dataset[, ifelse(ccheques_emitidos == 0, mcheques_emitidos := NA, mcheques_emitidos)]
	dataset[, ifelse(ccheques_depositados_rechazados == 0, mcheques_depositados_rechazados := NA, mcheques_depositados_rechazados)]
	dataset[, ifelse(ccheques_emitidos_rechazados == 0, mcheques_emitidos_rechazados := NA, mcheques_emitidos_rechazados)]
	dataset[, ifelse(tcallcenter == 0, ccallcenter_trx := NA, ccallcenter_trx)]
	dataset[, ifelse(thomebanking == 0, chomebanking_trx := NA, chomebanking_trx)]
	dataset[, ifelse(catm_trx == 0, matm := NA, matm)]
	dataset[, ifelse(catm_trx_other == 0, matm_other := NA, matm_other)]
	dataset[, ifelse(cmobile_app_trx == 0, cmobile_app_trx := NA, cmobile_app_trx)]
	dataset[, ifelse(ctarjeta_master == 0, Master_Finiciomora := NA, Master_Finiciomora)]
	dataset[, ifelse(ctarjeta_master == 0, Master_msaldototal := NA, Master_msaldototal)]
	dataset[, ifelse(ctarjeta_master == 0, Master_msaldopesos := NA, Master_msaldopesos)]
	dataset[, ifelse(ctarjeta_master == 0, Master_msaldodolares := NA, Master_msaldodolares)]
	dataset[, ifelse(ctarjeta_master == 0, Master_mconsumospesos := NA, Master_mconsumospesos)]
	dataset[, ifelse(ctarjeta_master == 0, Master_mconsumosdolares := NA, Master_mconsumosdolares)]
	dataset[, ifelse(ctarjeta_master == 0, Master_mlimitecompra := NA, Master_mlimitecompra)]
	dataset[, ifelse(ctarjeta_master == 0, Master_madelantopesos := NA, Master_madelantopesos)]
	dataset[, ifelse(ctarjeta_master == 0, Master_madelantodolares := NA, Master_madelantodolares)]
	dataset[, ifelse(ctarjeta_master == 0, Master_fultimo_cierre := NA, Master_fultimo_cierre)]
	dataset[, ifelse(ctarjeta_master == 0, Master_mpagado := NA, Master_mpagado)]
	dataset[, ifelse(ctarjeta_master == 0, Master_mpagospesos := NA, Master_mpagospesos)]
	dataset[, ifelse(ctarjeta_master == 0, Master_mpagosdolares := NA, Master_mpagosdolares)]
	dataset[, ifelse(ctarjeta_master == 0, Master_fechaalta := NA, Master_fechaalta)]
	dataset[, ifelse(ctarjeta_master == 0, Master_mconsumototal := NA, Master_mconsumototal)]
	dataset[, ifelse(ctarjeta_master == 0, Master_cconsumos := NA, Master_cconsumos)]
	dataset[, ifelse(ctarjeta_master == 0, Master_cadelantosefectivo := NA, Master_cadelantosefectivo)]
	dataset[, ifelse(ctarjeta_master == 0, Master_mpagominimo := NA, Master_mpagominimo)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_delinquency := NA, Visa_delinquency)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_status := NA, Visa_status)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_mfinanciacion_limite := NA, Visa_mfinanciacion_limite)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_Fvencimiento := NA, Visa_Fvencimiento)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_Finiciomora := NA, Visa_Finiciomora)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_msaldototal := NA, Visa_msaldototal)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_msaldopesos := NA, Visa_msaldopesos)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_msaldodolares := NA, Visa_msaldodolares)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_mconsumospesos := NA, Visa_mconsumospesos)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_mconsumosdolares := NA, Visa_mconsumosdolares)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_mlimitecompra := NA, Visa_mlimitecompra)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_madelantopesos := NA, Visa_madelantopesos)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_madelantodolares := NA, Visa_madelantodolares)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_fultimo_cierre := NA, Visa_fultimo_cierre)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_mpagado := NA, Visa_mpagado)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_mpagospesos := NA, Visa_mpagospesos)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_mpagosdolares := NA, Visa_mpagosdolares)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_fechaalta := NA, Visa_fechaalta)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_mconsumototal := NA, Visa_mconsumototal)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_cconsumos := NA, Visa_cconsumos)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_cadelantosefectivo := NA, Visa_cadelantosefectivo)]
