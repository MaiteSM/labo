...
agrego_NA <- function(dataset){
	gc()
	dataset[, ifelse(ccuenta_corriente == 0, mcuenta_corriente_adicional := NA)]
	dataset[, ifelse(ccuenta_corriente == 0, mcuenta_corriente := NA)]
	dataset[, ifelse(ccaja_ahorro == 0, mcaja_ahorro := NA)]
	dataset[, ifelse(ccaja_ahorro == 0, mcaja_ahorro_adicional := NA)]
	dataset[, ifelse(ctarjeta_debito == 0, ctarjeta_debito_trx := NA)]
	dataset[, ifelse(ctarjeta_debito == 0, mautoservicio := NA)]
	dataset[, ifelse(ctarjeta_visa == 0, ctarjeta_visa_trx := NA)]
	dataset[, ifelse(ctarjeta_visa == 0, mtarjeta_visa_consumo := NA)]
	dataset[, ifelse(ctarjeta_master == 0, ctarjeta_master_trx := NA)]
	dataset[, ifelse(ctarjeta_master == 0, mtarjeta_master_consumo := NA)]
	dataset[, ifelse(cprestamos_personales == 0, mprestamos_personales := NA)]
	dataset[, ifelse(cprestamos_prendarios == 0, mprestamos_prendarios := NA)]
	dataset[, ifelse(cprestamos_hipotecarios == 0, mprestamos_hipotecarios := NA)]
	dataset[, ifelse(cplazo_fijo == 0, mplazo_fijo_dolares := NA)]
	dataset[, ifelse(cplazo_fijo == 0, mplazo_fijo_pesos := NA)]
	dataset[, ifelse(cinversion1 == 0, minversion1_pesos := NA)]
	dataset[, ifelse(cinversion1 == 0, minversion1_dolares := NA)]
	dataset[, ifelse(cinversion2 == 0, minversion2 := NA)]
	dataset[, ifelse(cpayroll_trx == 0, mpayroll := NA)]
	dataset[, ifelse(cpayroll_trx == 0, mpayroll2 := NA)]
	dataset[, ifelse(cpayroll_trx == 0, cpayroll2_trx := NA)]
	dataset[, ifelse(ccuenta_debitos_automaticos == 0, mcuenta_debitos_automaticos := NA)]
	dataset[, ifelse(ctarjeta_visa_debitos_automaticos == 0, mtarjeta_visa_debitos_automaticos := NA)]
	dataset[, ifelse(ctarjeta_master_debitos_automaticos == 0, mttarjeta_master_debitos_automaticos := NA)]
	dataset[, ifelse(cpagodeservicios == 0, mpagodeservicios := NA)]
	dataset[, ifelse(cpagomiscuentas == 0, mpagomiscuentas := NA)]
	dataset[, ifelse(ccajeros_propios_descuentos == 0, mcajeros_propios_descuentos := NA)]
	dataset[, ifelse(ctarjeta_visa_descuentos == 0, mtarjeta_visa_descuentos := NA)]
	dataset[, ifelse(ctarjeta_master_descuentos == 0, mtarjeta_master_descuentos := NA)]
	dataset[, ifelse(ccomisiones_mantenimiento == 0, mcomisiones_mantenimiento := NA)]
	dataset[, ifelse(ccomisiones_otras == 0, mcomisiones_otras := NA)]
	dataset[, ifelse(cforex == 0, cforex_buy := NA)]
	dataset[, ifelse(cforex == 0, mforex_buy := NA)]
	dataset[, ifelse(cforex == 0, cforex_sell := NA)]
	dataset[, ifelse(cforex == 0, mforex_sell := NA)]
	dataset[, ifelse(cforex_buy == 0, mforex_buy := NA)]
	dataset[, ifelse(cforex_sell == 0, mforex_sell := NA)]
	dataset[, ifelse(ctransferencias_recibidas == 0, mtransferencias_recibidas := NA)]
	dataset[, ifelse(ctransferencias_emitidas == 0, mtransferencias_emitidas := NA)]
	dataset[, ifelse(cextraccion_autoservicio == 0, mextraccion_autoservicio := NA)]
	dataset[, ifelse(ccheques_depositados == 0, mcheques_depositados := NA)]
	dataset[, ifelse(ccheques_emitidos == 0, mcheques_emitidos := NA)]
	dataset[, ifelse(ccheques_depositados_rechazados == 0, mcheques_depositados_rechazados := NA)]
	dataset[, ifelse(ccheques_emitidos_rechazados == 0, mcheques_emitidos_rechazados := NA)]
	dataset[, ifelse(tcallcenter == 0, ccallcenter_trx := NA)]
	dataset[, ifelse(thomebanking == 0, chomebanking_trx := NA)]
	dataset[, ifelse(catm_trx == 0, matm := NA)]
	dataset[, ifelse(catm_trx_other == 0, matm_other := NA)]
	dataset[, ifelse(cmobile_app_trx == 0, cmobile_app_trx := NA)]
	dataset[, ifelse(ctarjeta_master == 0, Master_Finiciomora := NA)]
	dataset[, ifelse(ctarjeta_master == 0, Master_msaldototal := NA)]
	dataset[, ifelse(ctarjeta_master == 0, Master_msaldopesos := NA)]
	dataset[, ifelse(ctarjeta_master == 0, Master_msaldodolares := NA)]
	dataset[, ifelse(ctarjeta_master == 0, Master_mconsumospesos := NA)]
	dataset[, ifelse(ctarjeta_master == 0, Master_mconsumosdolares := NA)]
	dataset[, ifelse(ctarjeta_master == 0, Master_mlimitecompra := NA)]
	dataset[, ifelse(ctarjeta_master == 0, Master_madelantopesos := NA)]
	dataset[, ifelse(ctarjeta_master == 0, Master_madelantodolares := NA)]
	dataset[, ifelse(ctarjeta_master == 0, Master_fultimo_cierre := NA)]
	dataset[, ifelse(ctarjeta_master == 0, Master_mpagado := NA)]
	dataset[, ifelse(ctarjeta_master == 0, Master_mpagospesos := NA)]
	dataset[, ifelse(ctarjeta_master == 0, Master_mpagosdolares := NA)]
	dataset[, ifelse(ctarjeta_master == 0, Master_fechaalta := NA)]
	dataset[, ifelse(ctarjeta_master == 0, Master_mconsumototal := NA)]
	dataset[, ifelse(ctarjeta_master == 0, Master_cconsumos := NA)]
	dataset[, ifelse(ctarjeta_master == 0, Master_cadelantosefectivo := NA)]
	dataset[, ifelse(ctarjeta_master == 0, Master_mpagominimo := NA)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_delinquency := NA)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_status := NA)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_mfinanciacion_limite := NA)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_Fvencimiento := NA)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_Finiciomora := NA)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_msaldototal := NA)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_msaldopesos := NA)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_msaldodolares := NA)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_mconsumospesos := NA)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_mconsumosdolares := NA)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_mlimitecompra := NA)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_madelantopesos := NA)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_madelantodolares := NA)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_fultimo_cierre := NA)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_mpagado := NA)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_mpagospesos := NA)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_mpagosdolares := NA)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_fechaalta := NA)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_mconsumototal := NA)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_cconsumos := NA)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_cadelantosefectivo := NA)]
	dataset[, ifelse(ctarjeta_visa == 0, Visa_mpagominimo := NA)]
